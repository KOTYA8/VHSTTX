...
class TestLiveIterable(unittest.TestCase):

    def test_wrap_live_iterable_preserves_length(self):
        wrapped = teletext.cli.teletext.wrap_live_iterable(range(5))
        self.assertTrue(hasattr(wrapped, '__len__'))
        self.assertEqual(len(wrapped), 5)
        self.assertEqual(list(wrapped), [0, 1, 2, 3, 4])
...
class TestVBICropHelpers(unittest.TestCase):
    ...
    def test_parse_vbi_start_value_supports_relative_shift(self):
        parsed = teletext.cli.teletext.parse_vbi_start_value(None, None, ('1',))
        self.assertEqual(parsed, (8, 321))

    def test_parse_vbi_start_value_supports_explicit_pair(self):
        parsed = teletext.cli.teletext.parse_vbi_start_value(None, None, ('7', '320'))
        self.assertEqual(parsed, (7, 320))

    def test_parse_vbi_count_value_supports_total_frame_lines(self):
        parsed = teletext.cli.teletext.parse_vbi_count_value(None, None, ('34',))
        self.assertEqual(parsed, (17, 17))

    def test_parse_vbi_count_value_supports_explicit_pair(self):
        parsed = teletext.cli.teletext.parse_vbi_count_value(None, None, ('17', '17'))
        self.assertEqual(parsed, (17, 17))

    def test_default_vbi_start_for_bt8x8_uses_standard_start(self):
        config = Config(card='bt8x8')
        self.assertEqual(
            teletext.cli.teletext.default_vbi_start_for_config(config),
            teletext.cli.teletext.BT8X8_DEFAULT_VBI_START,
        )

    def test_default_vbi_count_for_bt8x8_uses_standard_count(self):
        config = Config(card='bt8x8')
        self.assertEqual(
    ...
    def test_apply_vbi_runtime_format_supports_vbi_count_without_vbi_start(self):
        config = Config(card='bt8x8')
        updated, original = teletext.cli.teletext.apply_vbi_runtime_format(
            input_handle=None,
            input_path='C:\\temp\\sample.vbi',
            config=config,
            vbi_start=None,
            vbi_count=(17, 17),
        )
    ...
    def test_apply_vbi_runtime_format_supports_vbi_start_without_vbi_count(self):
        config = Config(card='bt8x8')
        updated, original = teletext.cli.teletext.apply_vbi_runtime_format(
            input_handle=None,
            input_path='C:\\temp\\sample.vbi',
        ...
        self.assertIsNone(original)
        self.assertEqual(updated.field_lines, 16)
        self.assertEqual(tuple(updated.field_range), tuple(range(16)))
