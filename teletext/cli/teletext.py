...
try:
    ...
    from teletext.vbi import line as _vbi_line

    ADAPTIVE_THRESHOLD_COEFF_DEFAULT = getattr(_vbi_line, 'ADAPTIVE_THRESHOLD_COEFF_DEFAULT', 1.0)
    ADAPTIVE_THRESHOLD_DEFAULT = getattr(_vbi_line, 'ADAPTIVE_THRESHOLD_DEFAULT', 0)
    AUTO_LINE_ALIGN_DEFAULT = getattr(_vbi_line, 'AUTO_LINE_ALIGN_DEFAULT', 0)
    AUTO_GAIN_CONTRAST_COEFF_DEFAULT = getattr(_vbi_line, 'AUTO_GAIN_CONTRAST_COEFF_DEFAULT', 1.0)
    AUTO_GAIN_CONTRAST_DEFAULT = getattr(_vbi_line, 'AUTO_GAIN_CONTRAST_DEFAULT', 0)
    AUTO_BLACK_LEVEL_COEFF_DEFAULT = getattr(_vbi_line, 'AUTO_BLACK_LEVEL_COEFF_DEFAULT', 1.0)
    AUTO_BLACK_LEVEL_DEFAULT = getattr(_vbi_line, 'AUTO_BLACK_LEVEL_DEFAULT', 0)
    BRIGHTNESS_COEFF_DEFAULT = getattr(_vbi_line, 'BRIGHTNESS_COEFF_DEFAULT', 48.0)
    CLOCK_LOCK_DEFAULT = getattr(_vbi_line, 'CLOCK_LOCK_DEFAULT', 50)
    CLOCK_LOCK_COEFF_DEFAULT = getattr(_vbi_line, 'CLOCK_LOCK_COEFF_DEFAULT', 1.0)
    CONTRAST_COEFF_DEFAULT = getattr(_vbi_line, 'CONTRAST_COEFF_DEFAULT', 0.5)
    DROPOUT_REPAIR_DEFAULT = getattr(_vbi_line, 'DROPOUT_REPAIR_DEFAULT', 0)
    DROPOUT_REPAIR_COEFF_DEFAULT = getattr(_vbi_line, 'DROPOUT_REPAIR_COEFF_DEFAULT', 1.0)
    GAIN_COEFF_DEFAULT = getattr(_vbi_line, 'GAIN_COEFF_DEFAULT', 0.5)
    HEAD_SWITCHING_MASK_COEFF_DEFAULT = getattr(_vbi_line, 'HEAD_SWITCHING_MASK_COEFF_DEFAULT', 1.0)
    HEAD_SWITCHING_MASK_DEFAULT = getattr(_vbi_line, 'HEAD_SWITCHING_MASK_DEFAULT', 0)
    HUM_REMOVAL_COEFF_DEFAULT = getattr(_vbi_line, 'HUM_REMOVAL_COEFF_DEFAULT', 1.0)
    HUM_REMOVAL_DEFAULT = getattr(_vbi_line, 'HUM_REMOVAL_DEFAULT', 0)
    IMPULSE_FILTER_COEFF_DEFAULT = getattr(_vbi_line, 'IMPULSE_FILTER_COEFF_DEFAULT', 1.0)
    IMPULSE_FILTER_DEFAULT = getattr(_vbi_line, 'IMPULSE_FILTER_DEFAULT', 0)
    LINE_STABILIZATION_COEFF_DEFAULT = getattr(_vbi_line, 'LINE_STABILIZATION_COEFF_DEFAULT', 1.0)
    LINE_STABILIZATION_DEFAULT = getattr(_vbi_line, 'LINE_STABILIZATION_DEFAULT', 0)
    NOISE_REDUCTION_COEFF_DEFAULT = getattr(_vbi_line, 'NOISE_REDUCTION_COEFF_DEFAULT', 1.0)
    NOISE_REDUCTION_DEFAULT = getattr(_vbi_line, 'NOISE_REDUCTION_DEFAULT', 0)
    QUALITY_THRESHOLD_DEFAULT = getattr(_vbi_line, 'QUALITY_THRESHOLD_DEFAULT', 50)
    QUALITY_THRESHOLD_COEFF_DEFAULT = getattr(_vbi_line, 'QUALITY_THRESHOLD_COEFF_DEFAULT', 1.0)
    START_LOCK_DEFAULT = getattr(_vbi_line, 'START_LOCK_DEFAULT', 50)
    START_LOCK_COEFF_DEFAULT = getattr(_vbi_line, 'START_LOCK_COEFF_DEFAULT', 1.0)
    SHARPNESS_COEFF_DEFAULT = getattr(_vbi_line, 'SHARPNESS_COEFF_DEFAULT', 3.0)
    TEMPORAL_DENOISE_COEFF_DEFAULT = getattr(_vbi_line, 'TEMPORAL_DENOISE_COEFF_DEFAULT', 1.0)
    TEMPORAL_DENOISE_DEFAULT = getattr(_vbi_line, 'TEMPORAL_DENOISE_DEFAULT', 0)
    WOW_FLUTTER_COMPENSATION_DEFAULT = getattr(_vbi_line, 'WOW_FLUTTER_COMPENSATION_DEFAULT', 0)
    WOW_FLUTTER_COMPENSATION_COEFF_DEFAULT = getattr(_vbi_line, 'WOW_FLUTTER_COMPENSATION_COEFF_DEFAULT', 1.0)

    def normalise_per_line_shift_map(per_line_shift, maximum_line=32):
        helper = getattr(_vbi_line, 'normalise_per_line_shift_map', None)
        if callable(helper):
            return helper(per_line_shift, maximum_line=maximum_line)
        return {}
from teletext.vbi.rangeprofiles import (
    format_tuning_range_label,
    normalise_tuning_ranges,
    resolve_tuning_range,
    tuning_ranges_signature,
)
...
def _split_vbi_option_values(value):
    if value in (None, (), ''):
        return None
...
def parse_vbi_start_value(ctx, param, value):
    parts = _split_vbi_option_values(value)
    if not parts:
        return None
    if len(parts) == 1:
        try:
            shift = int(parts[0], 10)
        except ValueError as exc:
            raise click.BadParameter(f'Invalid VBI start shift: {parts[0]!r}.', ctx=ctx, param=param) from exc
        return (
            int(BT8X8_DEFAULT_VBI_START[0]) + shift,
            int(BT8X8_DEFAULT_VBI_START[1]) + shift,
        )
    if len(parts) == 2:
        try:
            return (int(parts[0], 10), int(parts[1], 10))
        except ValueError as exc:
            raise click.BadParameter(f'Invalid VBI start values: {parts!r}.', ctx=ctx, param=param) from exc
    raise click.BadParameter('Use -vs SHIFT or -vs START0 START1.', ctx=ctx, param=param)


def parse_vbi_count_value(ctx, param, value):
    parts = _split_vbi_option_values(value)
    if not parts:
        return None
    if len(parts) == 1:
        try:
            total = int(parts[0], 10)
        except ValueError as exc:
            raise click.BadParameter(f'Invalid VBI line count: {parts[0]!r}.', ctx=ctx, param=param) from exc
        if total < 2 or total % 2 != 0:
            raise click.BadParameter('Single-value -vc expects an even total frame line count, e.g. -vc 34.', ctx=ctx, param=param)
        field_count = total // 2
    ...
    if len(parts) == 2:
        try:
            ...
            raise click.BadParameter(f'Invalid VBI count values: {parts!r}.', ctx=ctx, param=param) from exc
    raise click.BadParameter('Use -vc TOTAL or -vc COUNT0 COUNT1.', ctx=ctx, param=param)


class VBIFormatOption(click.Option):
    ...
    def add_to_parser(self, parser, ctx):
        ...
        def parser_process(value, state):
            values = [value]
            if ',' not in str(value):
                while state.rargs and len(values) < 2:
                    candidate = state.rargs[0]
                    if not self._value_pattern.match(candidate):
                        break
...
def vbiformatparams(f):
    options = [
        ...
        click.option('-vc', '--vbi-count', cls=VBIFormatOption, type=click.UNPROCESSED, callback=parse_vbi_count_value, default=None,
                     metavar='LINES',
                     help='Raw VBI line count. Use -vc 34 for 17/17 lines, or -vc 17 17 for explicit field counts.'),
        click.option('-vs', '--vbi-start', cls=VBIFormatOption, type=click.UNPROCESSED, callback=parse_vbi_start_value, default=None,
                     metavar='SHIFT|START0 START1',
                     help='Raw VBI start. Use -vs 1 to shift standard BT878 start to 8/321, or -vs 7 320 for explicit field starts.'),
    ]
...
def frame_line_byte_ranges(config, lines):
    return tuple(ignored_frame_line_byte_ranges(config, lines))
...
def wrap_live_iterable(iterable, pause_controller=None, timer_seconds=None, label='teletext'):
    ...
    def _iter():
        nonlocal deadline, timer_notice_sent
    ...
    if hasattr(iterable, '__len__'):
        return LenWrapper(wrapped, len(iterable))
    return wrapped
...
def default_vbi_start_for_config(config, reference_format=None):
    if getattr(config, 'card', None) == 'bt8x8':
        return BT8X8_DEFAULT_VBI_START
    if reference_format is not None:
        return tuple(int(value) for value in reference_format['start'])
    return (0, 0)


def default_vbi_count_for_config(config, reference_format=None):
    if getattr(config, 'card', None) == 'bt8x8':
        return BT8X8_DEFAULT_VBI_COUNT
    if reference_format is not None:
        return tuple(int(value) for value in reference_format['count'])
    field_lines = int(getattr(config, 'field_lines', 0) or 0)
    return (field_lines, field_lines)
...
def apply_vbi_record_format(device, config, vbi_start=None, vbi_count=None):
    ...
    requested_start = tuple(int(value) for value in (
        default_vbi_start_for_config(config, reference_format=original_format)
        if vbi_start is None else vbi_start
    ))
    requested_count = tuple(int(value) for value in (
        default_vbi_count_for_config(config, reference_format=original_format)
        if vbi_count is None else vbi_count
    ))

    try:
        applied_format = set_vbi_capture_format(device, requested_start, requested_count)
    except OSError as exc:
...
def apply_vbi_runtime_format(input_handle, input_path, config, vbi_start=None, vbi_count=None):
    ...
    requested_format = {
        ...
        'sample_format': 0,
        'start': tuple(int(value) for value in (
            default_vbi_start_for_config(config)
            if vbi_start is None else vbi_start
        )),
        'count': tuple(int(value) for value in (
            default_vbi_count_for_config(config)
            if vbi_count is None else vbi_count
        )),
        'flags': 0,
    }
...
def normalise_line_insertions(line_insertions, total_frames, line_count=32):
    ...
    for insertion in line_insertions or ():
        ...
        if not source_path and mode != 'fill-selected':
            continue
        target_start = max(0, min(int(insertion.get('target_start_frame', 0)), maximum_frame))
        target_end = max(0, min(int(insertion.get('target_end_frame', target_start)), maximum_frame))
...
def config_with_output_line_count(config, output_line_count):
    output_line_count = max(int(output_line_count), 2)
    if output_line_count % 2 != 0:
        raise ValueError('Output line count must be an even number.')
    original_line_count = useful_frame_lines(config)
    if output_line_count < original_line_count:
        raise ValueError(f'Output line count must be at least {original_line_count}.')
    if output_line_count == original_line_count:
        return config
    field_lines = output_line_count // 2
    return Config(
        card=config.card,
        ...
        field_range=range(field_lines),
    )


def _copy_frame_to_output_layout(frame, source_config, output_config):
    if source_config is output_config:
        return bytes(frame)
    source_line_count = useful_frame_lines(source_config)
    output_line_count = useful_frame_lines(output_config)
    payload = bytearray(frame_size_for_config(output_config))
    shared_line_count = min(source_line_count, output_line_count)
    source_ranges = frame_line_byte_ranges(source_config, range(1, shared_line_count + 1))
    output_ranges = frame_line_byte_ranges(output_config, range(1, shared_line_count + 1))
    source_bytes = bytes(frame)
    for (source_start, source_end), (output_start, output_end) in zip(source_ranges, output_ranges):
        copy_size = min(output_end - output_start, source_end - source_start)
        if copy_size <= 0:
            continue
        payload[output_start:output_start + copy_size] = source_bytes[source_start:source_start + copy_size]
    return bytes(payload)


def _replace_frame_line_ranges(target_frame, source_frame, byte_ranges, preserve_tail=0):
    if not byte_ranges:
        return target_frame

    original_target = bytes(target_frame)
    target = bytearray(original_target)
    source = bytes(source_frame)
    payload_limit = max(min(len(target), len(source)) - max(int(preserve_tail), 0), 0)

    for start, end in byte_ranges:
        start = max(int(start), 0)
        end = max(int(end), start)
        if start >= payload_limit:
            continue
        end = min(end, payload_limit)
        target[start:end] = source[start:end]

    if preserve_tail:
        target[-preserve_tail:] = original_target[-preserve_tail:]
    return bytes(target)


def _copy_frame_line_pairs(target_frame, target_config, source_frame, source_config, line_pairs, preserve_tail=0):
    original_target = bytes(target_frame)
    target = bytearray(original_target)
    source = bytes(source_frame)
    payload_limit = max(min(len(target), len(source)) - max(int(preserve_tail), 0), 0)
    ...
    for source_line, target_line in line_pairs:
        source_ranges = frame_line_byte_ranges(source_config, (int(source_line),))
        target_ranges = frame_line_byte_ranges(target_config, (int(target_line),))
        if not source_ranges or not target_ranges:
            continue
        source_start, source_end = source_ranges[0]
        target_start, target_end = target_ranges[0]
        if target_start >= payload_limit or source_start >= payload_limit:
            continue
        copy_size = min(target_end - target_start, source_end - source_start, payload_limit - target_start, payload_limit - source_start)
        if copy_size <= 0:
            continue
        target[target_start:target_start + copy_size] = source[source_start:source_start + copy_size]
...
def _clear_frame_lines(frame, config, line_numbers, preserve_tail=0):
    original = bytes(frame)
    target = bytearray(original)
    payload_limit = max(len(target) - max(int(preserve_tail), 0), 0)
    for start, end in frame_line_byte_ranges(config, tuple(int(line) for line in line_numbers)):
        start = max(int(start), 0)
        end = min(max(int(end), start), payload_limit)
        if end > start:
            target[start:end] = b'\x00' * (end - start)
    if preserve_tail:
        target[-preserve_tail:] = original[-preserve_tail:]
    return bytes(target)


def _flat_line_bytes(reference_bytes, config):
    from teletext.vbi.line import samples_from_bytes, samples_to_bytes

    payload = bytes(reference_bytes)
    if not payload:
        return payload
    samples = np.asarray(samples_from_bytes(payload, config.dtype), dtype=np.float32)
    if samples.size == 0:
        return payload
    # Approximate the requested dark maroon preview tone (#2D100E) with
    # a stable low luminance level in VBI sample space.
    fill_value = 58.0
    flattened = np.full(samples.shape, fill_value, dtype=np.float32)
    return samples_to_bytes(flattened, config.dtype)


def _fill_frame_lines(frame, config, line_numbers, preserve_tail=0):
    original = bytes(frame)
    target = bytearray(original)
    payload_limit = max(len(target) - max(int(preserve_tail), 0), 0)
    for start, end in frame_line_byte_ranges(config, tuple(int(line) for line in line_numbers)):
        ...
        if end > start:
            target[start:end] = _flat_line_bytes(original[start:end], config)
    if preserve_tail:
        target[-preserve_tail:] = original[-preserve_tail:]
    return bytes(target)
...
def save_cropped_vbi(
    ...
    with open(input_path, 'rb') as source, open(output_path, 'wb') as output:
        ...
        for frame_index in range(start_frame, end_frame + 1):
            ...
            if len(frame) < frame_size:
                break
            effective_controls, effective_selection, effective_decoder_tuning, active_key, _active_index = resolve_tuning_range(
                frame_index,
                controls,
                line_selection,
                decoder_tuning,
            ...
            temporal_state = temporal_states.setdefault(active_key or '__base__', {})
            output.write(
                _processed_frame_for_output(
                    frame,
                    config,
                    effective_controls,
                    line_selection=effective_selection,
                    temporal_state=temporal_state,
...
def save_edited_vbi(
    ...
    frame_size = frame_size_for_config(config)
    input_line_count = useful_frame_lines(config)
    base_output_config = output_config or config
    if output_line_count is None:
        output_line_count = input_line_count
    output_config = config_with_output_line_count(base_output_config, output_line_count)
    cut_ranges = tuple(sorted(cut_ranges))
    insertions = tuple(sorted(insertions, key=lambda item: (int(item['after_frame']), item['path'])))
    total_input_frames = count_complete_frames(input_path, config)
    line_insertions = normalise_line_insertions(
        line_insertions,
        total_input_frames,
    ...
    tuning_ranges = normalise_tuning_ranges(
        ...
        line_count=useful_frame_lines(output_config),
    )
    temporal_states = {}
    preserve_tail = 4 if config.card == 'bt8x8' else 0
    source_handles = {}
    total_inserted_frames = sum(max(int(item['frame_count']), 0) for item in insertions)
    progress_total = max(int(total_input_frames) + int(total_inserted_frames), 1)
    progress_current = 0
    selected_lines = (
        current_line_selection(config) if line_selection is None else frozenset(int(line) for line in line_selection)
    )
    selected_lines = frozenset(
        sorted(set(int(line) for line in selected_lines) | {
            int(target_line)
            for insertion in line_insertions
            for _source_line, target_line in insertion.get('line_mapping', ())
            if 1 <= int(target_line) <= useful_frame_lines(output_config)
        })
    )
    ...
    def write_insertions(output, after_frame):
        nonlocal insertion_index, progress_current
        while insertion_index < len(insertions) and int(insertions[insertion_index]['after_frame']) == after_frame:
            ...
            with open(insertion['path'], 'rb') as insert_file:
                inserted_frame_index = int(after_frame)
                while True:
                    frame = insert_file.read(frame_size)
                    if len(frame) < frame_size:
                        break
                    frame = _copy_frame_to_output_layout(frame, config, output_config)
                    effective_controls, effective_selection, effective_decoder_tuning, active_key, _active_index = resolve_tuning_range(
                        inserted_frame_index,
                        controls,
                        selected_lines,
                        decoder_tuning,
                        tuning_ranges,
                    ...
                    inserted_frame_index += 1
                    progress_current += 1
                    if (progress_current % 16) == 0 or progress_current >= progress_total:
                        report_progress()
            insertion_index += 1

    def read_source_frame(path, frame_index):
        handle = source_handles.get(path)
        if handle is None:
            handle = open(path, 'rb')
            source_handles[path] = handle
    ...
    def apply_line_insertions(frame, frame_index):
        patched = frame
        for insertion in line_insertions:
            target_start = int(insertion['target_start_frame'])
            target_end = int(insertion['target_end_frame'])
            if frame_index < target_start or frame_index > target_end:
                continue
            mode = _normalise_line_insertion_mode(
                insertion.get('mode'),
                insertion.get('remove_source_lines', False),
            ...
            if mode == 'fill-selected':
                patched = _fill_frame_lines(
                    patched,
                    output_config,
                    tuple(int(target_line) for _source_line, target_line in line_pairs),
                    preserve_tail=preserve_tail,
                )
                continue
            relative_index = int(frame_index) - target_start
            source_frame_index = int(insertion['source_start_frame']) + relative_index
            if source_frame_index >= int(insertion['source_frame_count']):
                continue
            source_frame = read_source_frame(str(insertion['source_path']), source_frame_index)
            if source_frame is None:
                continue
            source_frame = _copy_frame_to_output_layout(source_frame, config, output_config)
            if mode == 'fill-source':
                patched = _fill_frame_lines(
                    patched,
                    output_config,
                    tuple(int(source_line) for source_line, _target_line in insertion.get('line_mapping', ())),
                    preserve_tail=preserve_tail,
                )
            patched = _copy_frame_line_pairs(
                patched,
                output_config,
                source_frame,
                output_config,
    ...
    try:
        with open(input_path, 'rb') as source, open(output_path, 'wb') as output:
            ...
            while True:
                ...
                if cut_index < len(cut_ranges):
                    ...
                    if cut_start <= frame_index <= cut_end:
                        ...
                        continue
                frame = _copy_frame_to_output_layout(frame, config, output_config)
                frame = apply_line_insertions(frame, frame_index)
                effective_controls, effective_selection, effective_decoder_tuning, active_key, _active_index = resolve_tuning_range(
                    frame_index,
                    controls,
                    selected_lines,
            ...
            while insertion_index < len(insertions):
                insertion = insertions[insertion_index]
                with open(insertion['path'], 'rb') as insert_file:
                    inserted_frame_index = total_input_frames
                    while True:
                        frame = insert_file.read(frame_size)
                        if len(frame) < frame_size:
                            break
...
def scan_vbicrop_error_ranges(
    ...
    *,
    ...
    controls = normalise_signal_controls_tuple(controls)
    selected_lines = None if line_selection is None else frozenset(int(line) for line in line_selection)
    tuning_ranges = normalise_tuning_ranges(
        tuning_ranges,
        total_frames=total_frames,
        line_count=max(useful_frame_lines(config), 32),
    ...
    with open(input_path, 'rb') as handle:
        for frame_index in range(total_frames):
            ...
            metric_lines = None if effective_selection is None else frozenset(int(line) for line in effective_selection)
            frame_metrics.append(_vbicrop_frame_error_metrics(frame_index, lines, selected_lines=metric_lines))
...
class VBIRepairDiagnostics:
    ...
    def _runtime(self):
        ...
        runtime['tuning_ranges'] = normalise_tuning_ranges(
            runtime.get('tuning_ranges', ()),
            line_count=max(useful_frame_lines(runtime['config']), 32),
        )
        return runtime
    ...
    def _decode_current_frame(self, frame_index, runtime):
        ...
        effective_controls, effective_selected_lines, effective_decoder_tuning, active_key, _active_index = resolve_tuning_range(
            frame_index,
            runtime['controls'],
            runtime['selected_lines'],
            runtime['decoder_tuning'],
            runtime.get('tuning_ranges', ()),
            line_count=max(useful_frame_lines(runtime['config']), 32),
        )
        temporal_state_key = active_key or '__base__'
        temporal_state = self._temporal_profile_states.setdefault(temporal_state_key, {})
        ...
        selected_lines = frozenset(int(line) for line in (effective_selected_lines or ()))
    ...
    def _render_all_row_zero_results(
        ...
        try:
            ...
            if self._all_row_zero_render_key != cache_key:
...
def _vbitool_impl(input_path, config, pause, tape_format, line_quality, clock_lock, start_lock, adaptive_threshold, dropout_repair, wow_flutter_compensation, auto_line_align, per_line_shift, show_quality, show_rejects, show_start_clock, show_clock_visuals, show_alignment_visuals, show_quality_meter, show_histogram_graph, show_eye_pattern, n_lines, ignore_lines, used_lines, brightness, sharpness, gain, contrast, impulse_filter, temporal_denoise, noise_reduction, hum_removal, auto_black_level, head_switching_mask, line_to_line_stabilization, auto_gain_contrast, vbi_tune, show_vitc=False, show_vitc_console=False):
    ...
    try:
        ...
        from teletext.gui.vbirepair import open_monitor_window
    except ModuleNotFoundError as e:
        if e.name == 'PyQt5':
    ...
    def current_tuning_range_snapshot():
        return {
    ...
    live_tuner = None
    viewer_process = None
    source_preview_process = None
    edited_preview_process = None
    edited_preview_path = None
    monitor_window = None

    def neutral_preview_controls():
        return current_signal_controls('50', '50', '50', '50', '0', '0', '0', '0', '0', '0', '0', '0')

    def output_base_config(output_card):
        output_card = str(output_card or state['config'].card)
        if output_card == state['config'].card:
            return state['config']
        return Config(card=output_card)

    def output_preview_config(output_card, output_line_count):
        return config_with_output_line_count(output_base_config(output_card), output_line_count)

    def close_preview_process(process):
        if process is not None and process.is_alive():
            process.terminate()
            process.join(timeout=1)

    def open_preview_process(preview_path, preview_config, *, start_frame=0, window_name='VBI Preview', controls_override=None, line_selection_override=None):
        preview_total_frames = count_complete_frames(preview_path, preview_config)
        if preview_total_frames <= 0:
            raise click.UsageError('Preview file does not contain any complete VBI frames.')
        preview_state = create_crop_state(
            total_frames=preview_total_frames,
            current_frame=max(min(int(start_frame), preview_total_frames - 1), 0),
            playing=False,
        )
        return launch_crop_viewer(
            input_path=preview_path,
            config=preview_config,
            crop_state=preview_state,
            total_frames=preview_total_frames,
            frame_rate=DEFAULT_FRAME_RATE,
            pause=True,
            tape_format=state['tape_format'],
            n_lines=useful_frame_lines(preview_config),
            live_tuner=None,
            fixed_controls=controls_override,
    ...
    diagnostics = VBIRepairDiagnostics(
        input_path,
        state_provider=vbitool_runtime_state,
        mode='deconvolve',
        eight_bit=False,
        n_lines=n_lines,
        page_history_frames=15,
    ...
    def open_monitor_callback():
        nonlocal monitor_window
        if monitor_window is not None:
            try:
                ...
                monitor_window = None
        monitor_window = open_monitor_window(
            state=crop_state,
            total_frames=total_frames,
            frame_rate=DEFAULT_FRAME_RATE,
            diagnostics_callback=diagnostics,
            viewer_process=lambda: viewer_process,
            title='Teletext Monitor - VBI Tool',
        )

    def apply_tuning_ranges(next_ranges):
        nonlocal tuning_ranges
        tuning_ranges = normalise_tuning_ranges(
            next_ranges,
            total_frames=total_frames,
            line_count=max(useful_frame_lines(state['config']), 32),
        )
        restart_viewer()

    def save_callback(output_path, cut_ranges, insertions, line_insertions, output_line_count, output_card, progress_callback=None):
        ...
        save_edited_vbi(
            ...
            output_config=output_base_config(output_card),
            progress_callback=progress_callback,
            tuning_ranges=tuning_ranges,
        )

    def preview_selected_callback(preview_path, start_frame=0, output_card=None):
        nonlocal source_preview_process
        close_preview_process(source_preview_process)
        current_controls_value = state['controls'] if live_tuner is None else live_tuner.values()
        current_line_selection_value = state['selected_lines'] if live_tuner is None else live_tuner.line_selection()
        source_preview_process = open_preview_process(
            preview_path,
            state['config'],
            start_frame=start_frame,
            window_name=f'VBI Source Preview - {os.path.basename(str(preview_path))}',
            controls_override=current_controls_value,
            line_selection_override=current_line_selection_value,
        )

    def preview_edited_callback(cut_ranges, insertions, line_insertions, output_line_count, output_card, progress_callback=None):
        nonlocal edited_preview_process, edited_preview_path
        current_controls_value = state['controls'] if live_tuner is None else live_tuner.values()
        current_line_selection_value = state['selected_lines'] if live_tuner is None else live_tuner.line_selection()
        current_decoder_tuning_value = build_decoder_tuning_state(state['config'], state['tape_format']) if live_tuner is None else (live_tuner.decoder_tuning() or build_decoder_tuning_state(state['config'], state['tape_format']))
        preview_config = output_preview_config(output_card, output_line_count)
        fd, preview_path = tempfile.mkstemp(prefix='vhsttx-vbitool-preview-', suffix='.vbi')
        os.close(fd)
        try:
            save_edited_vbi(
                input_path=input_path,
            ...
            try:
                ...
                pass
            raise
        close_preview_process(edited_preview_process)
        if edited_preview_path and os.path.exists(edited_preview_path):
            try:
                os.remove(edited_preview_path)
            except OSError:
                pass
        edited_preview_path = preview_path
        edited_preview_process = open_preview_process(
            preview_path,
            preview_config,
            start_frame=0,
            window_name=f'VBI Edited Preview - {os.path.basename(str(input_path))}',
            controls_override=neutral_preview_controls(),
            line_selection_override=None,
        )
    ...
    try:
        run_crop_window(
            ...
            output_card_names=tuple(Config.cards.keys()),
            output_card=state['config'].card,
            input_path=input_path,
            monitor_callback=open_monitor_callback,
            capture_tuning_range_callback=current_tuning_range_snapshot,
            tuning_ranges_changed_callback=apply_tuning_ranges,
            initial_tuning_ranges=tuning_ranges,
        )
    finally:
        diagnostics.close()
        if monitor_window is not None:
            try:
                ...
                pass
        close_preview_process(viewer_process)
        close_preview_process(source_preview_process)
        close_preview_process(edited_preview_process)
        if edited_preview_path and os.path.exists(edited_preview_path):
            try:
...
def vbirepair(input_path, config, pause, mode, eight_bit, tape_format, line_quality, clock_lock, start_lock, adaptive_threshold, dropout_repair, wow_flutter_compensation, auto_line_align, per_line_shift, show_quality, show_rejects, show_start_clock, show_clock_visuals, show_alignment_visuals, show_quality_meter, show_histogram_graph, show_eye_pattern, n_lines, ignore_lines, used_lines, brightness, sharpness, gain, contrast, impulse_filter, temporal_denoise, noise_reduction, hum_removal, auto_black_level, head_switching_mask, line_to_line_stabilization, auto_gain_contrast, fix_capture_card, vbi_tune, vbi_tune_live, show_vitc, show_vitc_console):
    ...
    try:
        ...
        from teletext.gui.vbirepair import open_monitor_window, run_repair_window
    except ModuleNotFoundError as e:
        if e.name == 'PyQt5':
    ...
    def current_tuning_range_snapshot():
        ...
        return {
            ...
            'line_selection': state['selected_lines'] if live_tuner is None else live_tuner.line_selection(),
    ...
    def open_monitor_callback():
        ...
        monitor_window = open_monitor_window(
            ...
            diagnostics_callback=diagnostics,
            viewer_process=lambda: viewer_process,
            title='Teletext Monitor - VBI Repair',
        )
...
def _launch_t42_tool_editor(input_path):
    ...
    def save_callback(output_path, final_entries, progress_callback=None):
        write_t42_entries(final_entries, output_path, progress_callback=progress_callback)
