...
if QtCore is not None:
    ...
    class RowOrderWidget(QtWidgets.QWidget):
        def __init__(self, parent=None, count=25, start_index=0):
            super().__init__(parent)
        ...
        def sizeHint(self):
            return QtCore.QSize(36, max(250, self._count * 10))

        def set_layout_metrics(self, *, count=None, start_index=None, top_padding=None, bottom_padding=None):
            if count is not None:
                self._count = max(1, int(count))
            if start_index is not None:
                self._start_index = int(start_index)
            if top_padding is not None:
                self._top_padding = max(0, int(round(top_padding)))
            if bottom_padding is not None:
                self._bottom_padding = max(0, int(round(bottom_padding)))
            self.update()
        ...
        def paintEvent(self, event):  # pragma: no cover - GUI paint path
            ...
            font = painter.font()
            available_height = max(1, self.height() - self._top_padding - self._bottom_padding)
            row_height = available_height / max(1, self._count)
            font.setPointSize(max(7, min(11, int(row_height * 0.55))))
            painter.setFont(font)
            metrics = painter.fontMetrics()
            step = available_height / max(1, self._count)
            for index in range(self._count):
                label = str(self._start_index + index)
                top = int(self._top_padding + (index + 0.5) * step - metrics.height() / 2)
                painter.drawText(0, top, self.width() - 4, metrics.height(), QtCore.Qt.AlignRight, label)
            painter.end()
    ...
    class HtmlPreviewDialog(QtWidgets.QDialog):
        def __init__(self, parent=None):
            ...
            self._row_order_toggle = QtWidgets.QCheckBox(self)
            self._row_order_toggle.hide()
            self._row_order_action = self._settings_menu.addAction('Row Order')
            self._row_order_action.setCheckable(True)
            self._row_order_action.toggled.connect(self._row_order_toggle.setChecked)
            self._row_order_toggle.toggled.connect(self._row_order_action.setChecked)
            self._row_order_toggle.toggled.connect(self._set_row_order_visible)
            ...
            preview_layout = QtWidgets.QHBoxLayout(self._preview_container)
            preview_layout.setContentsMargins(0, 0, 0, 0)
            preview_layout.setSpacing(6)

            self._row_order_widget = RowOrderWidget(self._preview_container)
            self._row_order_widget.hide()
            preview_layout.addWidget(self._row_order_widget, 0)

            self._preview_frame = QtWidgets.QWidget()
            preview_frame_layout = QtWidgets.QGridLayout(self._preview_frame)
            ...
            preview_frame_layout.addWidget(self._preview_stack, 0, 0)
            self._scanline_overlay = HtmlScanlineOverlay(self._preview_frame)
            preview_frame_layout.addWidget(self._scanline_overlay, 0, 0)
        ...
        def _set_row_order_visible(self, enabled):
            self._row_order_widget.setVisible(bool(enabled))
            self._update_row_order_geometry()

        def _update_row_order_geometry(self):
            height = self._preview_frame.height() or self._preview_stack.height()
            if height > 0:
                self._row_order_widget.setFixedHeight(height)
            self._row_order_widget.set_layout_metrics(count=25, start_index=0, top_padding=0, bottom_padding=0)
    ...
    class TeletextViewerWindow(QtWidgets.QMainWindow):
        ...
        def _build_ui(self):
            ...
            self._row_order_widget = RowOrderWidget(self._decoder_area)
            self._row_order_widget.hide()
            decoder_layout.addWidget(self._row_order_widget, 0)
        ...
        def _sync_decoder_size(self):
            if self._fullscreen_button.isChecked():
                ...
                self._decoder_widget.setFixedSize(self._decoder.size())
                self._row_order_widget.setFixedHeight(self._decoder_widget.height())
                self._row_order_widget.set_layout_metrics(count=25, start_index=0, top_padding=0, bottom_padding=0)
            ...
            self._decoder.fullscreenstretch = False
            self._decoder_widget.setFixedSize(self._decoder.size())
            border = float(self._decoder_widget.rootObject().property('effectiveBorderSize') or 0)
            self._row_order_widget.setFixedHeight(self._decoder_widget.height())
            self._row_order_widget.set_layout_metrics(
                count=25,
                start_index=0,
                top_padding=border,
