...
from teletext.vbi.rangeprofiles import normalise_tuning_ranges, resolve_tuning_range
...
def _run_crop_viewer(
    ...
    tuning_ranges = normalise_tuning_ranges(
        ...
        line_count=max(int(frame_line_count), 32),
    )

    def current_frame_index():
        return max(min(int(crop_shared_values[CropPlaybackStateProxy.CURRENT_INDEX]), total_frames - 1), 0)

    def base_signal_controls():
        if signal_controls is not None:
            return tuple(_live_signal_controls(signal_controls))
        return tuple(fixed_controls or DEFAULT_CONTROLS)

    def base_decoder_tuning():
        if decoder_tuning is not None:
            tuning = _live_decoder_tuning(decoder_tuning, tape_formats)
            if tuning is not None:
                return tuning
        return fixed_decoder_tuning

    def base_line_selection():
        if line_selection is not None:
            return _live_line_selection(line_selection, frame_line_count)
        if fixed_line_selection is None:
            return None
        return frozenset(int(line) for line in fixed_line_selection)

    def resolve_effective_state():
        return resolve_tuning_range(
            current_frame_index(),
            base_signal_controls(),
            base_line_selection(),
            base_decoder_tuning(),
            tuning_ranges,
            line_count=max(int(frame_line_count), 32),
    ...
    initial_controls, initial_line_selection, initial_decoder_tuning, _active_key, _active_index = resolve_effective_state()
    ...
    if signal_controls is None and not tuning_ranges:
        signal_controls_cb = None
    else:
        signal_controls_cb = lambda: tuple(resolve_effective_state()[0])

    if decoder_tuning is None and fixed_decoder_tuning is None and not tuning_ranges:
        decoder_tuning_cb = None
    else:
        decoder_tuning_cb = lambda: resolve_effective_state()[2]

    if line_selection is None and fixed_line_selection is None and not tuning_ranges:
